...
export class Polygon{
	...
	public get points():Array<Point>{
 	...
 	public initialize_from_array(points: Array<any>): void{
		...
		this._radius = BJMath.dist(points[0].x, points[0].y, this.centroid.x, this.centroid.y);
	}
	...
	public generate_inner_grid(resolution:number): Array<Point>{
		...
		for (let i = 0; i < this._vertices.length; i++) {
			...
			for (let j = 1; j < resolution; j++) {
				...
				pts.push(new Point(BJMath.lerp(curr.x, center.x, t), BJMath.lerp(curr.y, center.y, t)));
				pts.push(new Point(BJMath.lerp(curr.x, next.x, t), BJMath.lerp(curr.y, next.y, t)));
			}	
		}
		return pts;
	};
